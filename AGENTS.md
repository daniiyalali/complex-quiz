...
# Local toolchain (2026-06-22)

This machine has **no system Node.js** — nothing on `PATH`, no nvm/volta/fnm/brew. A standalone **Node v22.14.0** (darwin-arm64) is vendored at `./.localnode/`. Before any `node`/`npm`/`next` command, prefix `PATH` from the project root:
```bash
export PATH="$PWD/.localnode/bin:$PATH"
```
`./.localnode/` is a runtime only, not an app dependency — safe to delete once Node is installed globally. (There is also no `.gitignore`; a future `git init` would otherwise commit `node_modules`/`.next`/`.localnode`.)
