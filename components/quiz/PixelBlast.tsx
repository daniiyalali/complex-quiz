...
/**
 * PixelBlast — Bayer-dithered pixel-noise background (React Bits port).
 *
 * TypeScript port of the original JSX component. Inspired by:
 *   https://github.com/zavalit/bayer-dithering-webgl-demo
 *
 * Used as an alternate celebratory backdrop behind the badge unlock moment
 * on the v3 trial preview. CSS lives in `PixelBlast.module.css`.
 */
...
void main(){
  ...
  // sRGB gamma correction — linear to sRGB
  vec3 srgbColor = mix(
    color * 12.92,
...
// ─── Liquid touch-trail texture (used when `liquid` is true) ───────────────
    ...
    if (mustReinit) {
      ...
      if (noiseAmount > 0) {
        ...
        const noiseEffect = new Effect(
          "NoiseEffect",
          /* glsl */ `uniform float uTime; uniform float uAmount; float hash(vec2 p){ return fract(sin(dot(p, vec2(127.1,311.7))) * 43758.5453);} void mainUv(inout vec2 uv){} void mainImage(const in vec4 inputColor,const in vec2 uv,out vec4 outputColor){ float n=hash(floor(uv*vec2(1920.0,1080.0))+floor(uTime*60.0)); float g=(n-0.5)*uAmount; outputColor=inputColor+vec4(vec3(g),0.0);} `,
          {
            uniforms: new Map<string, THREE.Uniform>([
              ["uTime", new THREE.Uniform(0)],
